    ...
    $('#submit_button').on('click', function () {
        ...
        horns_req.done(function (data) {
            ...
            for (let key in horns) {
                ...
                let horn_div=$('<div class="horn_div" id="'+id_name+'"></div>').appendTo('#horns_div')
                id_name='#'+id_name;
                horn_div.append('<p class="horn_name"><a href=' + horns[key]['horn_link'] + '>' + horns[key]['name'] + '</a></p>')
                //notes
                ...
                $.get(API_URL + '/get_effects_from_horn', { 'horn': JSON.stringify(horns[key]['name']) }).done(function (data2) {
                    let eff_ul=$('<ul>').appendTo(horn_div);
                    let horn_effects=data2['response']['effects'];
                    horn_effects.forEach(function(effect){
                        eff_ul.append('<li>' + ((effect.hasOwnProperty('effect_urls'))?('<a href='+effect['effect_urls'][0]+'>'+effect['name']+'</a>'):(effect['name']))+'</li>');
                    });
